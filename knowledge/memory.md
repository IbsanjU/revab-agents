...
- **Orchestrator**: file queue `.queue/pending|running|done|failed`; handlers in `agents/registry.ts` — `run-bdd`/`generate-report` require `payload.project`; `typecheck` stays scoped to this framework repo; CLI `npm run task -- enqueue|status|types`; worker `npm run worker`.
