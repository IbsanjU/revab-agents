...
- **MCP servers** (local HTTP, stateless, `.vscode/mcp.json`):
  ...
  - `github` :7320 — `github_search_code`, `github_search_repos`, `github_search_issues`, `github_search_commits`, `github_get_file` — read-only, repo-agnostic (org/repo are call-time qualifiers, not manifest-resolved); defaults to `GITHUB_ORG` when a call doesn't name its own org/repo
  ...
  - `media` :7319 — `get_file_metadata` (image/PDF/DOCX/generic metadata as JSON), `read_pdf_text`, `read_docx_text`, `create_pdf`, `create_docx` (styled, accent-color support) — resolves paths against this repo by default or a manifest `project`'s repoPath if given; uses `pdf-parse`, `mammoth`, `image-size`, `file-type`, `pdfkit`, `docx`
  ...
  - Shared infra: `mcp-servers/shared/server.ts` (`startMcpHttpServer`, `textResult`, `errorResult`), `mcp-servers/shared/http.ts` (`apiGet`, `apiPost`, `apiPut`, `apiDelete`, plus exported `buildUrl`/`handleJson` reused by other HTTP-backed servers), `mcp-servers/shared/githubHttp.ts` (`githubGet`, GITHUB_TOKEN-based auth, `defaultGithubOrg`), `utils/fsSafety.ts` (`resolveWithinRoot`, the reusable path-containment check), `utils/saveSuggestion.ts` (`suggestProjectFolder`/`buildSaveConfirmationPrompt` for the ask-before-save pattern), `utils/confluenceMacros.ts` (`expandConfluenceMacros`), `utils/jiraLinks.ts` (`buildJiraIssueUrl`, shared by jira/jtmf servers)
...
- **Skills**: `skills/*/SKILL.md` — `analyze-test-failures`, `detect-execution-convention`, `upload-to-jtmf`, `update-jira-epic`, `extract-requirements-from-image`, `extract-requirements-from-video`, `consolidate-project-report`, `build-test-plan-interactive`, `search-across-sources` (federates Jira/Confluence/JTMF/GitHub topic search).
...
- **Media limits**: no OCR/speech-to-text MCP tool exists yet — `extract-requirements-from-image`/`extract-requirements-from-video` skills rely on native vision capability or manually supplied transcripts until one is built. Confluence viewer analytics needs Cloud Premium. The `media` server covers images/PDF/DOCX metadata + text extraction + styled PDF/DOCX generation, but not audio/video transcription.
...
- Copy `.env.example` → `.env` and fill in Atlassian auth (`ATLASSIAN_EMAIL`, `ATLASSIAN_API_TOKEN`, base URLs, `JTMF_STEPS_FIELD`) and GitHub auth (`GITHUB_TOKEN`, `GITHUB_ORG`).
