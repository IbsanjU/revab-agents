...
10. **Dry-run first for writes**: all Jira/JTMF write MCP tools (`jira_update_issue`, `jira_transition_issue`, `jtmf_create_test_case`, `jtmf_update_test_case`) default to `dryRun: true` and must be confirmed by the user before setting `dryRun: false`.
